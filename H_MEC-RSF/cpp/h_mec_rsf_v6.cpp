...
// set timestep limit
const int num_timesteps = 12; // very small number for testing
...
const int savestep = 5;  // storage periodicity
...
int main() {
    ...
    if (timestep > 0) {
        ...
        MatXd dummy_mat = read_matrix(filename, group_matrix, "Dummy_mat", dims);

        hsize_t dim[1] = {num_timesteps};
        VecXd dummy_vec = read_matrix(filename, group_vector, "Dummy_vec", dim);
    ...
    for (; timestep <= num_timesteps; timestep++) {
        ...
        if (timestep % savestep == 0) {
            ...
            add_group(save_file_name, group_matrix);
            add_group(save_file_name, group_vector);

            // save dummy matrix and vector for testing
            hsize_t dims[2] = {Ny, Nx};
            MatXd dummy_mat = MatXd::Constant(Ny, Nx, 1.);
            add_matrix(save_file_name, group_matrix, dummy_mat, "Dummy_mat", dims);
            
            hsize_t dim[1] = {num_timesteps};
            VecXd dummy_vec = VecXd::Constant(num_timesteps, 1.);
            add_vector(save_file_name, group_vector, dummy_vec, "Dummy_vec", dim);
        }
    }
    return 0;
}