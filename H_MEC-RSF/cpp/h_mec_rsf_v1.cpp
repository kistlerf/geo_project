...
const double dxms = xsize / Nx_markers; // Standard marker horizontal step
const double dyms = ysize / Ny_markers; // Standard marker vertical step
...
MatrixXd SIIB(Ny, Nx);
MatrixXd PEFFB0(Ny, Nx);
MatrixXd PEFFB(Ny, Nx);
MatrixXd PEFFB1(Ny, Nx);
...
int main() {
    ...
    if (timestep > 0) {
        ...
        for (int jm = 0; jm < Nx_markers; jm++) {
            for (int im = 0; im < Ny_markers; im++, m++) { // Update marker index each time
                ...
                fricttm(m) = tensile;
                porm(m) = .01 * (1 + .0 * (rand() % 1 - .5)); // multiplication with 0 ???
                ...
                if (ym(m) < upper_block || ym(m) > lower_block) {
                    ...
                    cohescm(m) = cohes * 1e3;
                    cohestm(m) = cohes * 1e3;
                    frictcm(m) = friction;       // does not change the value of that variable ???
                    dilatcm(m) = dilatation;     // does not change the value of that variable ???
                    fricttm(m) = tensile;        // does not change the value of that variable ???
                }
            }
        }
        L.setZero();
    ...
    for (; timestep <= num_timesteps; timestep++) {
        ...
        for (iterstep = 0; iterstep < niterglobal; iterstep++) {
            ...
            if (dt > 2e+5) {
                pfscale = ETADX.block(1, 0, Ny - 2, Nx - 1).minCoeff() * dx * 1e+17 / pow(dt, 2);
            } else {
                pfscale = ETADX.block(1, 0, Ny - 2, Nx - 1).minCoeff() * dx;
            }
            ...
            if (!yn && (ynpl == 0 || (DSYLSQ(iterstep) < errmin && iterstep > 1 && abs(vratio) < vratiomax))) {
                ynstop = true;
            } else {
                // Recomputing ETA
                for (int i = 0; i < Ny; i++) {
