...
int main() {
    ...
    for (; timestep <= num_timesteps; timestep++) {
        ...
        for (iterstep = 0; iterstep < niterglobal; iterstep++) {
            ...
            int na = 100000000; // Some maximum
            int ia[N];    // first coloumn of each row
            int ja[nnz];    // coloumn position of each koef in the respective row from left to right
            double a[nnz];  // value of each koef
            ...
            int phase = 11; // ???
            PARDISO(pardiso_pt, &maxfct, &mnum, &mtype, &phase, &na, a, ia, ja, &int_dummy, &nrhs, iparm, &msglvl, &double_dummy, &double_dummy, &error, dparm);
            ...
            PARDISO(pardiso_pt, &maxfct, &mnum, &mtype, &phase, &na, a, ia, ja, &int_dummy, &nrhs, iparm, &msglvl, b, x, &error, dparm);
