...
int main() {
        ...
        for (int jm = 0; jm < Nx_markers; jm++) {
            for (int im = 0; im < Ny_markers; im++, m++) { // Update marker index each time
                ...
                if (ym(m) < upper_block || ym(m) > lower_block) {
                    ...
                    // Does not change value of variables
                    // frictcm(m) = friction;
    ...
    for (; timestep <= num_timesteps; timestep++) {
        ...
        for (iterstep = 0; iterstep < niterglobal; iterstep++) {
            ...
            for (int i = 0; i < Ny; i++) {
                for (int j = 0; j < Nx; j++) {
                    ETA(i, j) = ETA(i, j) < etamincur ? etamincur : ETA(i, j);
            ...
            for (int i = 1; i < Ny; i++) {
                for (int j = 1; j < Nx; j++) {
                    ...
                    // Check viscosity
                    ETAP(i, j) = ETAP(i, j) < etamincur ? etamincur : ETAP(i, j);
                    ETAB(i, j) = ETAB(i, j) * POR(i, j) < etamincur ? etamincur / POR(i, j) : ETAB(i, j);
            ...
            pfscale = dt > 2e5 ? ETADX.block(1, 0, Ny - 1, Nx).minCoeff() * dx * 1e17 / pow(dt, 2) : ETADX.block(1, 0, Ny - 1, Nx).minCoeff() * dx;
            ...
            for (int j = 0; j < Nx1; j++) {
                for (int i = 0; i < Ny1; i++) {
                    ...
                    if (i == 0 || i == Ny || j == 0 || j >= Nx - 1) {
                        ...
                        //             vxs4
                        // Viscosity
                        ETAXY.segment(0, 4) << ETA(i - 1, j), ETA(i, j), ETAP(i, j), ETAP(i, j + 1);
                        // Shear modulus
                        GXY.segment(0, 4) << GGG(i - 1, j), GGG(i, j), GGGP(i, j), GGGP(i, j + 1);
                        // Viscoelasticity factor
                        KXY_vec(0) = dt * GXY(0) / (dt * GXY(0) + ETAXY(0));
                        KXY_vec(1) = dt * GXY(1) / (dt * GXY(1) + ETAXY(1));
                        KXY_vec(2) = dt * GXY(2) / (dt * GXY(2) + ETAXY(2));
                        KXY_vec(3) = dt * GXY(3) / (dt * GXY(3) + ETAXY(3));
                        // Numerical viscosity
                        ETAXY(0) *= KXY_vec(0);
                        ETAXY(1) *= KXY_vec(1);
                        ETAXY(2) *= KXY_vec(2);
                        ETAXY(3) *= KXY_vec(3);
                        // Numerical stresses
                        SXY_vec(0) = SXY0(i - 1, j) * (1 - KXY_vec(0));
                        SXY_vec(1) = SXY0(i, j) * (1 - KXY_vec(1));
                        SXY_vec(2) = SXX0(i, j) * (1 - KXY_vec(2));
                        SXY_vec(3) = SXX0(i, j + 1) * (1 - KXY_vec(3));
                        // Density derivatives
                        dRHOdx = (RHOX(i, j + 1) - RHOX(i, j - 1)) / (2. * dx);
                        dRHOdy = (RHO(i, j) - RHO(i - 1, j)) / dy;
                        // Left part
                        double dx2 = pow(dx, 2), dy2 = pow(dy, 2);
                        Trip.push_back(Trp(kx, kx, -4. / 3. * (ETAXY(2) + ETAXY(3)) / dx2 - (ETAXY(0) + ETAXY(1)) / dy2 - gx * dt * dRHOdx - ascale * RHOX(i, j) / dt)); //vxs3
                        Trip.push_back(Trp(kx, kx - Ny1 * 6, 4. / 3. * ETAXY(2) / dx2)); //vxs1
                        Trip.push_back(Trp(kx, kx + Ny1 * 6, 4. / 3. * ETAXY(3) / dx2)); //vxs5
                        Trip.push_back(Trp(kx, kx - 6, ETAXY(0) / dy2)); //vxs2
                        Trip.push_back(Trp(kx, kx + 6, ETAXY(1) / dy2)); //vxs4
                        double gx_dt_dRHOdy = gx * dt * dRHOdy / 4.;
                        double dx_dy = dx * dy;
                        Trip.push_back(Trp(kx, ky - 6, ETAXY(0) / dx_dy - 2. / 3. * ETAXY(2) / dx_dy - gx_dt_dRHOdy)); //vys1
                        Trip.push_back(Trp(kx, ky, -ETAXY(1) / dx_dy + 2. / 3. * ETAXY(2) / dx_dy - gx_dt_dRHOdy)); //vys2
                        Trip.push_back(Trp(kx, ky - 6 + Ny1 * 6, -ETAXY(0) / dx_dy + 2. / 3. * ETAXY(3) / dx_dy - gx_dt_dRHOdy)); //vys3
                        Trip.push_back(Trp(kx, ky + Ny1 * 6, ETAXY(1) / dx_dy - 2. / 3. * ETAXY(3) / dx_dy - gx_dt_dRHOdy)); //vys4
                        Trip.push_back(Trp(kx, kp, ptscale / dx)); //Pt1'
                        Trip.push_back(Trp(kx, kp + Ny1 * 6, -ptscale / dx)); //Pt2'
                        // Right part
                        R(kx) = -RHOX(i, j) * (ascale * VX0(i, j) / dt + gx) - (SXY_vec(3) - SXY_vec(2)) / dx - (SXY_vec(1) - SXY_vec(0)) / dy;
                    }
                    ...
                    if (j == 0 || j == Nx || i == 0 || i >= Ny - 1) {
                        ...
                        //               vys4
                        // Viscosity
                        ETAXY.segment(0, 2) << ETA(i, j - 1), ETA(i, j);
                        ETAXY.segment(4, 2) << ETAP(i, j), ETAP(i + 1, j);
                        // Shear modulus
                        GXY.segment(0, 2) << GGG(i, j - 1), GGG(i, j);
                        GXY.segment(4, 2) << GGGP(i, j), GGGP(i + 1, j);
                        // Viscoelasticity factor
                        KXY_vec(0) = dt * GXY(0) / (dt * GXY(0) + ETAXY(0));
                        KXY_vec(1) = dt * GXY(1) / (dt * GXY(1) + ETAXY(1));
                        KXY_vec(4) = dt * GXY(4) / (dt * GXY(4) + ETAXY(4));
                        KXY_vec(5) = dt * GXY(5) / (dt * GXY(5) + ETAXY(5));
                        // Numerical viscosity
                        ETAXY(0) *= KXY_vec(0);
                        ETAXY(1) *= KXY_vec(1);
                        ETAXY(4) *= KXY_vec(4);
                        ETAXY(5) *= KXY_vec(5);
                        // Numerical stresses
                        SXY_vec(0) = SXY0(i, j - 1) * (1 - KXY_vec(0));
                        SXY_vec(1) = SXY0(i, j) * (1 - KXY_vec(1));
                        SXY_vec(4) = SYY0(i, j) * (1 - KXY_vec(4));
                        SXY_vec(5) = SYY0(i + 1, j) * (1 - KXY_vec(5));
                        // Density derivatives
                        dRHOdy = (RHOY(i + 1, j) - RHOY(i - 1, j)) / 2. / dy;
                        dRHOdx = (RHO(i, j) - RHO(i, j - 1)) / dx;
                        // Left part
                        double dx2 = pow(dx, 2), dy2 = pow(dy, 2);
                        Trip.push_back(Trp(ky, ky, -4. / 3. * (ETAXY(4) + ETAXY(5)) / dy2 - (ETAXY(0) + ETAXY(1)) / dx2 - gy * dt * dRHOdy - ascale * RHOY(i, j) / dt)); //vys3
                        Trip.push_back(Trp(ky, ky - Ny1 * 6, ETAXY(0) / dx2)); //vys1
                        Trip.push_back(Trp(ky, ky + Ny1 * 6, ETAXY(1) / dx2)); //vys5
                        Trip.push_back(Trp(ky, ky - 6, 4. / 3. * ETAXY(4) / dy2)); //vys2
                        Trip.push_back(Trp(ky, ky + 6, 4. / 3. * ETAXY(5) / dy2)); //vys4
                        double gy_dt_dRHOdx = gy * dt * dRHOdx / 4.;
                        double dx_dy = dx * dy;
                        Trip.push_back(Trp(ky, kx - Ny1 * 6, ETAXY(0) / dx_dy - 2. / 3. * ETAXY(4) / dx_dy - gy_dt_dRHOdx)); //vxs1
                        Trip.push_back(Trp(ky, kx + 6 - Ny1 * 6, -ETAXY(0) / dx_dy + 2. / 3. * ETAXY(5) / dx_dy - gy_dt_dRHOdx)); //vxs2
                        Trip.push_back(Trp(ky, kx, -ETAXY(1) / dx_dy + 2. / 3. * ETAXY(4) / dx_dy - gy_dt_dRHOdx)); //vxs3
                        Trip.push_back(Trp(ky, kx + 6, ETAXY(1) / dx_dy - 2. / 3. * ETAXY(5) / dx_dy - gy_dt_dRHOdx)); //vxs4
                        Trip.push_back(Trp(ky, kp, ptscale / dy)); //Pt1'
                        Trip.push_back(Trp(ky, kp + 6, -ptscale / dy)); //Pt2'
                        // Right part
                        R(ky) = -RHOY(i, j) * (ascale * VY0(i, j) / dt + gy) - (SXY_vec(5) - SXY_vec(4)) / dy - (SXY_vec(1) - SXY_vec(0)) / dx;
                    }
            ...
            for (int i = 1; i < Ny; i++) {
                for (int j = 1; j < Nx; j++) {
                    double sxy_ij = SXY(i, j), sxy_i1j = SXY(i - 1, j), sxy_ij1 = SXY(i, j - 1), sxy_i1j1 = SXY(i - 1, j - 1);
                    ...
                    EXYVP2 = (pow(sxy_ij / eta_ij_2, 2) + pow(sxy_i1j / eta_i1j_2, 2) + pow(sxy_ij1 / eta_ij1_2, 2) + pow(sxy_i1j1 / eta_i1j1_2, 2)) / 4.;
                    ...
                    SXY(1) = (pow(sxy_ij, 2) + pow(sxy_i1j, 2) + pow(sxy_ij1, 2) + pow(sxy_i1j1, 2)) / 4.;
                    SII(i, j) = sqrt(0.5 * (pow(sxx_ij, 2) + pow(syy_ij, 2)) + SXY(1));
                    
                    // Dissipation
                    DISXY = (pow(sxy_ij, 2) / eta_ij_2 + pow(sxy_i1j, 2) / eta_i1j_2 + pow(sxy_ij1, 2) / eta_ij1_2 + pow(sxy_i1j1, 2) / eta_i1j1_2) / 4.;
        ...
        for (int i = 1; i < Ny; i++) {
            for (int j = 1; j < Nx; j++) {
                ...
                // SXY term is averaged from four surrounding basic nodes
                SXY(1) = (pow(sxy_ij, 2) + pow(sxy_i1j, 2) + pow(sxy_ij1, 2) + pow(sxy_i1j1, 2)) / 4.;
                SII(i, j) = sqrt(.5 * (pow(SXX(i, j), 2) + pow(SYY(i, j), 2)) + SXY(1));
