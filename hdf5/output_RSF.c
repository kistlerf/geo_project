...
for(il=0; il<xnumx; il++)
  	{fprintf(fld,"%e ", gx[il]);}
fclose(fld);
sprintf(fileTxt,"eachdt_ab_%s0.txt",name);
fld = fopen(fileTxt,"a");
for(il=0; il<xnumx; il++)
  	{fprintf(fld,"%e ", as[il*ynumy+(output_line)]-bs[il*ynumy+(output_line)]);}
fclose(fld);
}

// --- Horizontal velocity data ---
sprintf(fileTxt,"eachdt_%s0.txt",name);
fld = fopen(fileTxt,"a");
fprintf(fld," %.15e %e ",timesum,timestep);
for(il=0; il<ynumy; il++)
  	{fprintf(fld,"%e ", vx[(xnumx-1)/2*ynumy+il]);}
fprintf(fld," \n ");
fclose(fld);

// ---- Second invariant of stress
sprintf(fileTxt,"eachdt_sii_%s0.txt",name);
fld = fopen(fileTxt,"a");
fprintf(fld," %.15e %e ",timesum,timestep);
for(il=0; il<xnumx; il++)
	{ fprintf(fld,"%f ", pow(sxxs[il*ynumy+(output_line)]*sxxs[il*ynumy+(output_line)]+sxy[il*ynumy+(output_line)]*sxy[il*ynumy+(output_line)],0.5)  ); }
fprintf(fld," \n ");
fclose(fld);
...
// --- Pressure data ---
sprintf(fileTxt,"eachdt_pr_%s0.txt",name);
fld = fopen(fileTxt,"a");
fprintf(fld," %.15e %e ",timesum,timestep);
for(il=0; il<xnumx; il++)
        {fprintf(fld,"%e ", prs[il*ynumy+(output_line)] );}
fprintf(fld," \n ");
fclose(fld);

// --- Cohesion data ---
sprintf(fileTxt,"eachdt_pe_%s0.txt",name);
fld = fopen(fileTxt,"a");
fprintf(fld," %.15e %e ",timesum,timestep);
for(il=0; il<xnumx; il++)
        {fprintf(fld,"%.10e ", pes[il*ynumy+(output_line)] );}
fprintf(fld," \n ");
fclose(fld);
...
// --- Marker track data ---
sprintf(fileTxt,"eachdt_markertrack_%s0.txt",name);
fld = fopen(fileTxt,"a");
for (il=0; il<mintpmax; il++)
	{ fprintf(fld,"%.15e %.15e %.15e %e %e %e %e %e %e %e %e \n", timesum, markx[mtrack[il]], marky[mtrack[il]], markp[mtrack[il]],markxx[mtrack[il]],markxy[mtrack[il]],markexx[mtrack[il]],markexy[mtrack[il]],markv[mtrack[il]],markphi[mtrack[il]],timestep); }
fclose(fld);
